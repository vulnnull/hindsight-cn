...
|------|---------|-----------------|
| `include_data` | `true` | Documents, raw chunks, extracted facts, consolidated observations, entities and links, attachments **and their bytes**, the curation archive of invalidated facts, the operations log, and the maintenance queues. |
| `include_bank_config` | `true` | The bank's config overrides, mental models and their refresh history, knowledge pages, directives, and webhooks. |
