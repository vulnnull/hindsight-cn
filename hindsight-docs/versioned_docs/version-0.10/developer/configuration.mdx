...
A [bank alias](./api/memory-banks.mdx#aliases) is an extra id a bank answers to, resolved to the bank's real id on the way in. That lookup happens on every bank-scoped request, so it is cached per `(schema, alias)` — misses included, since most requests name a real bank and would otherwise pay a query that can only answer "not an alias".

The TTL is therefore how long a **newly added alias stays invisible**, and a **removed one keeps routing**, on API replicas other than the one that made the change. Lower it if you add aliases and cut traffic over to them in the same breath; raise it to save reads if your aliases are long-lived.

| Variable | Description | Default |
|----------|-------------|---------|
| `HINDSIGHT_API_BANK_ALIAS_CACHE_TTL_SECONDS` | Time-to-live (seconds) for alias→bank resolution. `0` disables caching, so every request resolves against the database. | `10` |
| `HINDSIGHT_API_BANK_ALIAS_CACHE_MAX_ENTRIES` | Maximum number of cached `(schema, alias)` entries before LRU eviction. | `2048` |
